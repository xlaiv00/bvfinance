...
interface Toast { id: number; message: string; type: string; undoFn?: () => void }
let _add: ((t: Omit<Toast,'id'>) => void) | null = null

export function toast(message: string, type = 'success', undoFn?: () => void) {
  if (_add) _add({ message, type, undoFn })
}
